...
public class gamevent : MonoBehaviour {
...
// Panel for the end question
	...
	void Start () {
		...
		//get save data
		xMap = Map1_0.x_Save_Pos;
		yMap = Map1_0.y_Save_Pos;
		now = Map1_0.now_Save_Pos;
		speed = 0f;
		//place character and camera
		xC = Map1_0.character_position[now].x;
		xCam = Map1_0.camera_position[now].x;
		zC = Map1_0.character_position[now].z;
		zCam = Map1_0.camera_position[now].z;
		//initial target as now position
		character.transform.position = Map1_0.character_position[now];
		cam_place.transform.position = Map1_0.camera_position[now];		
	...
	void move(){
		...
		if(p == 0){
			...
			if(Map1_0.world1[xMap,yMap+1] == now+1){
				...
				now++;
	   			p--;
	   		}else if(Map1_0.world1[xMap,yMap-1] == now+1){
    			...
    			now++;
  	  			p--;
    		}else if(Map1_0.world1[xMap+1,yMap] == now+1){
	...
	public void tryAns(){
		...
		clue_num.text = Map1_0.ans_cluenum;
	}
    ...
    private void entergame(){
    	...
    	save = true;
    	if(!savedata.gameEntered){
	    	savedata.entergame();
	    	switch(Map1_0.world1_game[now]){
	    		case 1:
    ...
    public void showclue(){
    	clue_L.text = Map1_0.cluelist_L;
    	clue_R.text = Map1_0.cluelist_R;
    	if(open_clue){
    		clue_panel.SetActive(false);
    		open_clue = false;	
